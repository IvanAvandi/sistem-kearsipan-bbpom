<nav class="flex" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-2">
        ...
        {{-- REQ 1: Hanya tampilkan Dashboard jika kita BERADA di halaman Dashboard --}}
        @if (empty($links))
            <li class="inline-flex items-center">
                <span class="inline-flex items-center text-sm font-medium text-gray-700">
                    {{-- Ikon Home (dari sidebar) --}}
        ...
        {{-- REQ 1 & 2: Loop untuk halaman selain Dashboard --}}
        @if (!empty($links))
            @foreach ($links as $label => $url)
                <li>
                    <div class="flex items-center">
                        
                        {{-- Jangan tampilkan separator (>) untuk item pertama --}}
                        @if (!$loop->first)
                            <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        @endif
                        
                        {{-- REQ 2: LOGIKA IKON HANYA UNTUK ITEM PERTAMA --}}
                        @php
                        ...
                        @if ($url === '#' || $loop->last)
                            {{-- Tautan Terakhir (Aktif) --}}
                            {{-- Hapus margin jika ini item pertama --}}
                            <span class="text-sm font-medium text-gray-700 inline-flex items-center @if(!$loop->first) ml-1 md:ml-2 @endif">
                                {!! $iconSvg !!} {{ $label }}
                            </span>
                        @else
                            {{-- Tautan Biasa --}}
                            {{-- Hapus margin jika ini item pertama --}}
                            <a href="{{ $url }}" class="text-sm font-medium text-gray-500 hover:text-indigo-600 inline-flex items-center @if(!$loop->first) ml-1 md:ml-2 @endif">
                                {!! $iconSvg !!} {{ $label }}
                            </a>
                        @endif
                    </div>
