<div class="p-6 bg-white border border-gray-200 rounded-lg shadow-md">
    ...
    <div class="mt-6 flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
        <div class="relative w-full sm:w-1/3">
            ...
            <x-input type="text" wire:model.debounce.300ms="search" placeholder="Cari nama atau kategori..." class="pl-10 w-full" />
        </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($items as $item)
                    <tr class="odd:bg-white even:bg-gray-50 align-middle">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{{ $item->kategori }}</td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">
                            <a href="{{ Storage::url($item->file_path) }}" target="_blank" class="text-blue-500 hover:underline" title="{{ $item->nama_file }}">
                                {{ $item->nama_file }}
                            </a>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-center text-sm font-medium">
                            <div class="flex justify-center items-center space-x-4">
                                <a href="{{ route('admin.templates.edit', $item->id) }}" title="Edit" class="text-gray-500 hover:text-indigo-600">
                                ...
                                <button wire:click.prevent="delete({{ $item->id }})" onclick="confirm('Anda yakin ingin menghapus Format ini?') || event.stopImmediatePropagation()" title="Hapus" class="text-gray-500 hover:text-red-600">
