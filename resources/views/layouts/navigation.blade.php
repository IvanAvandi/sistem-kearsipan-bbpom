<div class="flex flex-col h-full">
    ...
    <div class="flex-shrink-0">
        ...
        <div class="flex items-center h-16 px-3 border-b border-gray-700">
            <a href="{{ route('dashboard') }}" class="flex items-center flex-shrink-0">
                <img src="{{ asset('images/logo-bbpom.png') }}" alt="Logo BBPOM" 
                     class="w-15 h-12">
            </a>
    ...
    {{-- BAGIAN TENGAH: NAVIGASI UTAMA (BISA SCROLL) --}}
    <nav class="mt-2 px-4 flex-grow overflow-y-auto">
        
        {{-- Link Dashboard --}}
        <a href="{{ route('dashboard') }}" class="flex items-center p-3 text-gray-400 rounded-md hover:bg-gray-700 hover:text-white transition-colors duration-200 {{ request()->routeIs('dashboard') ? 'bg-gray-700 text-white' : '' }}">
            <div class="flex-shrink-0">
        ...
        {{-- Link Daftar Arsip --}}
        <a href="{{ route('arsip.index') }}" class="mt-2 flex items-center p-3 text-gray-400 rounded-md hover:bg-gray-700 hover:text-white transition-colors duration-200 {{ request()->routeIs('arsip.*') ? 'bg-gray-700 text-white' : '' }}">
            <div class="flex-shrink-0">
        ...
        {{-- =================== MENU USUL MUSNAH / REVIEW PINDAH =================== --}}
        @if(Auth::user()->role == 'Admin')
            {{-- Link Usul Musnah (ADMIN) --}}
            <a href="{{ route('admin.usulan-pemusnahan.index') }}" class="mt-2 flex items-center p-3 text-gray-400 rounded-md hover:bg-gray-700 hover:text-white transition-colors duration-200 {{ request()->routeIs('admin.usulan-pemusnahan.*') ? 'bg-gray-700 text-white' : '' }}">
                <div class="flex-shrink-0">
            ...
            {{-- Link Review Pindah (ADMIN) --}}
            <a href="{{ route('admin.usul-pindah-review.index') }}" class="mt-2 flex items-center p-3 text-gray-400 rounded-md hover:bg-gray-700 hover:text-white transition-colors duration-200 {{ request()->routeIs('admin.usul-pindah-review.*') || request()->routeIs('admin.usul-pindah.show') ? 'bg-gray-700 text-white' : '' }}">
                <div class="flex-shrink-0">
        ...
        @else
            {{-- Link Usul Pindah (ARSIPARIS) --}}
            <a href="{{ route('usul-pindah.index') }}" class="mt-2 flex items-center p-3 text-gray-400 rounded-md hover:bg-gray-700 hover:text-white transition-colors duration-200 {{ request()->routeIs('usul-pindah.*') ? 'bg-gray-700 text-white' : '' }}">
                <div class="flex-shrink-0">
                    <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 4H6a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-2m-4-1v8m0 0l3-3m-3 3L9 8m-5 5h2.586a1 1 0 01.707.293l2.414 2.414a1 1 0 001.414 0l2.414-2.414a1 1 0 01.707-.293H17" /></svg>
                </div>
        ...
        {{-- Link Format TND --}}
        <a href="{{ route('pusat-template.index') }}" class="mt-2 flex items-center p-3 text-gray-400 rounded-md hover:bg-gray-700 hover:text-white transition-colors duration-200 {{ request()->routeIs('pusat-template.*') ? 'bg-gray-700 text-white' : '' }}">
            <div class="flex-shrink-0">
        ...
        {{-- Link Pengaturan --}}
        @if(Auth::user()->role == 'Admin')
        <a href="{{ route('admin.pengaturan.index') }}" class="mt-2 flex items-center p-3 text-gray-400 rounded-md hover:bg-gray-700 hover:text-white transition-colors duration-200 {{ request()->routeIs(['admin.pengaturan.*', 'admin.klasifikasi.*', 'admin.bentuk-naskah.*', 'admin.link-terkait.*', 'admin.templates.*', 'admin.users.*']) ? 'bg-gray-700 text-white' : '' }}">
            <div class="flex-shrink-0">
    ...
    {{-- BAGIAN BAWAH (TIDAK BISA SCROLL) --}}
    <div class="flex-shrink-0">
        <div x-data="{ open: false }" class="relative border-t border-gray-700">
            <button @click="open = !open" class="flex items-center w-full p-5 hover:bg-gray-700 focus:outline-none">
                ...
                <div class="ml-3 overflow-hidden whitespace-nowrap" x-show="isSidebarOpen">
                    <div class="font-medium text-base text-left text-gray-200">{{ Auth::user()->name }}</div>
